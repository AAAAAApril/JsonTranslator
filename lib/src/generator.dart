...
class Generator {
  ...
  /// 开始生成
  /// 第一版（翻译完成之后统一写入文件）
  Future<void> generateAsyncV1(PubspecConfig config) async {
    //读取源文件
    ...
    for (var element in languageFile.entries) {
      ...
      ///写入 json 到文件
      await element.value.writeAsString(createFileContentByJson(languageJson[language]!));
  ...
  ///第二版（每翻译完成一个语言就写入文件）
  Future<void> generateAsyncV2(PubspecConfig config) async {
    //读取源文件
    final File sourceFile = File(config.sourceFilePath);
    if (!(await sourceFile.exists())) {
      throw Exception('未找到源语言文件！');
    ...
    for (var language in config.targetLanguages) {
      ...
      ///翻译完一个语言之后，立即将结果写入文件
      await targetFile.writeAsString(createFileContentByJson(targetJson));
